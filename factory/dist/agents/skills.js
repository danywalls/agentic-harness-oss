 ...
 */
export function detectSkills(title, body) {
 ...
 */
export function ensureSkillsInstalled(skills, log) {
    const installed = [];
    for (const skill of skills) {
        try {
            // Check if already installed by looking at the global skills dir
            const skillName = skill.includes('@') ? skill.split('@')[1] : skill;
            const checkResult = execSync(`ls ~/.agents/skills/${skillName} 2>/dev/null`, { encoding: 'utf8', timeout: 5000 }).trim();
            if (checkResult) {
                // Already installed
                continue;
            }
        ...
        try {
            ...
            execSync(`npx skills add ${skill} -g -a claude-code -y 2>/dev/null`, { encoding: 'utf8', timeout: 30000, stdio: 'pipe' });
 ...
 */
export function prepareSkillsForIssue(issueTitle, issueBody, station, log) {
    // Only install skills for build-related stations
    if (!['build', 'qa', 'bugfix'].includes(station))
        return;

...
import { getSubmissionForIssue, pushToThread, pushChangeRequestStatus, } from '../notify/supabase.js';
...
const stuckSkipCounts = new Map();
// ─── Phase 2 auto-pickup ──────────────────────────────────────────────────────
function getPhase2Checked() {
    try {
        return JSON.parse(readFileSync(PHASE2_CHECKED_FILE, 'utf8'));
    }
    catch {
        return [];
    }
}
function markPhase2Checked(issueNumber) {
...
async function autoQueuePhase2(issueNumber, deps) {
    try {
        const checked = getPhase2Checked();
        if (checked.includes(issueNumber))
            return;
        const allOpen = JSON.parse(execSync(`gh issue list --state open --limit 100 --repo ${deps.REPO} --json number,title,labels 2>/dev/null`, { encoding: 'utf8', timeout: 30000 }));
        const phase2 = allOpen.filter((i) => i.title.includes(`[Phase 2] #${issueNumber}`) &&
...
async function syncDoneStations(deps) {
    if (!deps.SUPABASE_URL)
        return; // standalone mode
    try {
        const doneIssues = JSON.parse(execSync(`gh issue list --label "station:done" --state open --limit 20 --repo ${deps.REPO} --json number,title,body,labels,url 2>/dev/null`, { encoding: 'utf8', timeout: 30000 }));
        for (const issue of doneIssues) {
            const enriched = enrichIssue(issue);
            if (enriched.isChangeRequest) {
                const crs = await fetch(`${deps.SUPABASE_URL}/rest/v1/change_requests?github_issue_number=eq.${issue.number}&select=id,status`, { headers: { apikey: deps.SUPABASE_KEY, Authorization: `Bearer ${deps.SUPABASE_KEY}` } }).then((r) => r.json()).catch(() => []);
            ...
            const subs = await fetch(`${deps.SUPABASE_URL}/rest/v1/submissions?github_issue_number=eq.${issue.number}&select=id,station`, { headers: { apikey: deps.SUPABASE_KEY, Authorization: `Bearer ${deps.SUPABASE_KEY}` } }).then((r) => r.json()).catch(() => []);
...
async function spawnForStation(issue, stationId, deps, ctx, taskFn) {
    ...
    getSubmissionForIssue(rawIssue.number, deps.SUPABASE_URL, deps.SUPABASE_KEY)
        .then((sub) => {
        if (sub?.id) {
            pushToThread(sub.id, 'status_update', { type: 'status_update', station: stationId }, statusMessages[stationId] ?? `Processing at ${stationId}...`, deps.FACTORY_APP_URL, deps.FACTORY_SECRET, deps.log);
        }
    })
        .catch(() => null);
            ...
            if (!process) {
                ...
                if (station instanceof BuildStation && station.designAction) {
                    ...
                    if (action === 'reject-design') {
                        ...
                        try {
                            const result = JSON.parse(execSync(`gh issue view ${issueNumber} --repo ${deps.REPO} --json comments`, { encoding: 'utf8', timeout: 15000 }));
    ...
    if (added < deps.MAX_TASKS_PER_RUN) {
        ...
        if (qaStation) {
            ...
            for (const raw of qaDirectIssues) {
                if (added >= deps.MAX_TASKS_PER_RUN)
                    break;
                if (deps.countActiveLocks('qa') >= qaStation.concurrency)
                    break;
                const issue = enrichIssue(raw);
                ...
                if (deps.isLocked(key)) {
                    deps.log(`QA already locked for #${raw.number}, skipping`);
                    continue;
                }
                if (deps.isInCrashBackoff(key))
                    continue;
 ...
 */
export async function tickV2(deps) {

...
import { tickV2 } from './pipeline/runner.js';
...
const lockManager = new LockManagerImpl(LOCK_FILE, AGENT_ACTIVITY_FILE, log, backoffManager.getMap(), (m) => backoffManager.save(m), (reason) => keyManager.rotateKey(reason), (logPath) => keyManager.checkLogForKeyError(logPath), (issueNum, station, logFile) => writeTokenUsageAsync(issueNum, station, logFile, SUPABASE_URL, SUPABASE_KEY, log), (msg) => notifyDiscord(msg, DISCORD_WEBHOOK_URL, log));
// ─── Load config.json ─────────────────────────────────────────────────────────
let config;
try {
    config = loadConfig();
...
function loadPipelinesConfig() {
    ...
    const pipelinesFilePath = config.pipelinesFile
        ? resolve(configDir, config.pipelinesFile)
...
const depsV2 = {
    ...
    setLock: (key, meta) => lockManager.setLock(key, meta),
...
async function main() {
    ...
    if (USE_CLAUDE_CLI) {
        ...
        if (!kv.ok) {
            ...
            await notifyDiscord(`🔴 Factory key invalid: \`${kv.reason}\` — no agents spawned. Run \`claude setup-token\` and update factory-keys.json.`, DISCORD_WEBHOOK_URL, log).catch(() => { });
            return;
        }
